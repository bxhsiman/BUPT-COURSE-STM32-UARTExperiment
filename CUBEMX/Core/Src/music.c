...
void music_process( void )
{
	...
	if ( g_music_flag == 1 )
	{
		...
		HAL_TIM_PWM_Stop( &htim2 ,TIM_CHANNEL_1 );
		__HAL_TIM_SET_AUTORELOAD( &htim2 , g_music_freq[g_music_tone[g_music_index]]-1 );
		__HAL_TIM_SET_COUNTER( &htim2 , 0 );
		__HAL_TIM_SET_COMPARE( &htim2 ,TIM_CHANNEL_1,g_music_freq[g_music_tone[g_music_index]]*0.05 );
		HAL_TIM_PWM_Start( &htim2 ,TIM_CHANNEL_1 );
		
		//HAL_TIM_Base_Stop_IT( &htim3 );
		__HAL_TIM_SET_AUTORELOAD( &htim3 , g_music_timer_set );
